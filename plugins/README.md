...
| [obsidian](./obsidian/) | Obsidian vault integration with wikilink-safe hooks and note management commands | **Commands:** `/new-note`, `/daily`, `/find-broken-links` - Create notes, manage daily notes, and audit wikilink integrity<br>**Hook:** PreToolUse - Warns before `mv` or `rm` on `.md` files to protect wikilink references<br>**Skill:** `obsidian-syntax` - Auto-applied rules for wikilinks, embeds, callouts, frontmatter, and file safety |
